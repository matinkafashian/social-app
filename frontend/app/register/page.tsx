  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-pink-50 to-white">
      <form onSubmit={onSubmit} className="w-full max-w-md bg-white rounded-2xl shadow p-6 sm:p-8">
        ...
        <h2 className="text-center text-lg font-medium mb-4">Create your account</h2>

        <div className="space-y-4">
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d="M12 12a5 5 0 100-10 5 5 0 000 10z"/><path fillRule="evenodd" d="M.458 20.042C1.732 15.4 6.314 12 12 12s10.268 3.4 11.542 8.042A1 1 0 0122.571 22H1.43a1 1 0 01-.972-1.958z" clipRule="evenodd"/></svg>
            </span>
            <input value={username} onChange={e=>setUsername(e.target.value)} placeholder="Choose a username" className="w-full border rounded-xl pl-10 pr-3 py-3" required />
          </div>
          ...
          <div className="relative">
            ...
            <input type={showPwd?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Create a password" className="w-full border rounded-xl pl-10 pr-10 py-3" required />
          ...
          <div className="relative">
            ...
            <input type={showConfirm?"text":"password"} value={confirm} onChange={e=>setConfirm(e.target.value)} placeholder="Confirm your password" className="w-full border rounded-xl pl-10 pr-10 py-3" required />
          ...
          <label className="flex items-center gap-2 text-sm text-gray-600 select-none">
            ...
            I agree to the <a href="#" className="mx-1 font-medium" style={{color:"#E85A7E"}}>Terms of Service</a> and <a href="#" className="font-medium" style={{color:"#E85A7E"}}>Privacy Policy</a>
          </label>
        </div>

        {error && <p className="text-center text-sm mt-2" style={{color:"#E85A7E"}}>{error}</p>}

        <button disabled={loading || !agree} className="w-full rounded-xl px-4 py-3 mt-4 btn-primary disabled:opacity-60">
          {loading?"Creating...":"Create Account"}
        </button>

        <p className="text-center text-sm text-gray-600 mt-4">Already have an account? <a href="/login" className="font-medium" style={{color:"#E85A7E"}}>Sign in</a></p>
      </form>
    </div>
  )
}
